	...
	<div>
		...
		<section class="article-detail con-min-width">
			...
			<form name="doReplyWrite" method="POST" action="">
				<input type="hidden" name="redirectUrl" value="${Util.getNewUrl(currentUrl, 'focusReplyId', '[NEW_REPLY_ID]')}" /> <input type="hidden" name="articleId" value="${article.id}" /> <input type="hidden" name="loginedMemberId" value="${loginedMemberId}" />
...
<hr />
<div class="reply-box-list">
	<div class="reply-box" data-id="${{replyId}}">
		<div class="reply-box__body">
			<div class="reply-box__body__info flex flex-ai-end">
				<div>${{replyWriter}}</div>
				<div class="reply-box__body__date">${{replyRegDate}}</div>
				<div class="reply-box__body__rcm${{replyId}} flex flex-jc-end flex-g-1">
					<div>
						<c:set var="likePointed" value="false" />
						<a href="#none" onclick="callDoReplyLike('reply', ${{replyId}});"> 
							<c:forEach items="${likes}" var="like">
								<c:if test="${like.relId == ${{replyId}} && like.memberId == loginedMemberId}">
									<c:if test="${like.point == 1}">
										<i id="replyLike${{replyId}}" class="fas fa-thumbs-up"></i>
										<c:set var="likePointed" value="true" />
									</c:if>
								</c:if>
							</c:forEach> 
							<c:if test="${likePointed == false}">
								<i id="replyLike${{replyId}}" class="far fa-thumbs-up"></i>
								<c:set var="likePointed" value="false" />
							</c:if> <span class="reply-like-point${{replyId}}">[${{replyLikePoint}}]</span>
						</a>
					</div>
					<div>
						<c:set var="likePointed" value="false" />
						<a href="#none" onclick="callDoReplyDislike('reply', ${{replyId}});"> 
							<c:forEach items="${likes}" var="like" varStatus="status">
								<c:if test="${like.relId == reply.id && like.memberId == loginedMemberId}">
									<c:if test="${like.point == -1}">
										<i id="replyDislike${{replyId}}" class="fas fa-thumbs-down"></i>
										<c:set var="likePointed" value="true" />
									</c:if>
								</c:if>
							</c:forEach> 
							<c:if test="${likePointed == false}">
								<i id="replyDislike${{replyId}}" class="far fa-thumbs-down"></i>
								<c:set var="likePointed" value="false" />
							</c:if> <span class="reply-dislike-point${{replyId}}">[${{replyDislikePoint}}]</span>
						</a>
					</div>
				</div>
			</div>
			<div class="reply-box__body__body">${{replyBody}}</div>
			<div class="reply-box__function">
				<form action="../reply/doReplyWrite" method="POST" onsubmit="DoReReplyWriteForm__submit(this); return false;">
					<input type="hidden" name="redirectUrl" value="${Util.getNewUrl(currentUrl, 'focusReplyId', '[NEW_REPLY_ID]')}" /> 
					<input type="hidden" name="articleId" value="${{replyArticleId}}" /> 
					<input type="hidden" name="parentReplyId" value="${{replyId}}" /> 
					<input type="hidden" name="parentReplyMmeber" value="@${{replyWriter}}::  " />
					<div class="flex flex-jc-end">
						<c:if test="${loginedMemberId == ${{replyMemberId}}}">
							<a class="modify input" href="javascript:toggleLayer('${{replyId}}modify');">수정</a>
							<a class="del input" onclick="return confirm('정말로 삭제하시겠습니까?')" href="../reply/doReplyDelete?id=${{replyId}}&articleId=${{replyArticleId}}">삭제</a>
						</c:if>
						<a href="javascript:toggleLayer('${{replyId}}xx');">답글</a>
					</div>
					<div class="re-reply-box" id="${{replyId}}xx" style="display: none;">
						<div>&#11177;${{replyWriter}}:: ${{replyWriter}}님께 댓글쓰기</div>
						<div class="reply-box flex flex-ai-c">
							<div class="reply-box__textarea flex-g-1">
								<textarea name="reReplyBody" placeholder="내용을 입력해 주세요."></textarea>
							</div>
				...
				<form action="../reply/doReplyModify" method="POST" onsubmit="DoReplyModifyForm__submit(this); return false;">
					<input type="hidden" name="redirectUrl" value="${Util.getNewUrl(currentUrl, 'focusReplyId', '[NEW_REPLY_ID]')}" /> <input type="hidden" name="articleId" value="${{replyArticleId}}" /> 
					<input type="hidden" name="replyId" value="${{replyId}}" />
					<div class="re-reply-box" id="${{replyId}}modify" style="display: none;">
						<div>댓글 수정</div>
						<div class="reply-box flex">
							<div class="reply-box__textarea flex-g-1">
								<textarea name="replyBody" placeholder="내용을 입력해 주세요.">${reply.body}</textarea>
